...
const HamiltonSlideInButton = ({
  ...
  return (
    <Link
      ...
      title={`${title} | Select Location | Hamilton`}
      aria-label={`${title} | Select Location | Hamilton`}
    >
      <div
        className={`${componentName}_HAMILTON_IMAGE_OVERLAY absolute top-0 left-0 w-full h-full bg-gray-900/50 z-[-1] group-hover:bg-gray-900/0 duration-[var(--main-duration)] rounded-lg`}
      ></div>
      <Image
        src={private_events_hamilton_two}
        title={`${title} | Select Location | Hamilton Image`}
        alt={`${title} | Select Location | Hamilton Image`}
        fill
